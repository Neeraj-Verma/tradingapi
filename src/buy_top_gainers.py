"""
Buy Top Gainers - Identifies and buys top gaining stocks with GTT OCO protection

This script:
1. Fetches OHLC data for NIFTY 50 stocks (or custom list)
2. Calculates % change from previous close
3. Ranks stocks by gain percentage
4. Places buy orders for top N gainers
5. Places GTT OCO orders (Stop Loss + Target) for protection

Usage:
    python buy_top_gainers.py                     # Dry run, show top 10 gainers
    python buy_top_gainers.py --execute           # Buy top 5 gainers with GTT
    python buy_top_gainers.py --top 10 --execute  # Buy top 10 gainers
    python buy_top_gainers.py --amount 50000      # Rs. 50,000 per stock
    python buy_top_gainers.py --min-gain 2        # Only stocks with 2%+ gain
    python buy_top_gainers.py --sl-pct 1 --target-pct 2  # 1% SL, 2% Target
    python buy_top_gainers.py --no-gtt            # Buy without GTT protection
"""
...
def get_top_gainers(kite: KiteConnect, symbols: List[str], exchange: str = "NSE") -> List[dict]:
    """
    Get stocks sorted by gain percentage (descending).
    ...
    try:
        ...
        # Sort by change_pct descending (top gainers first)
...
def main():
    parser = argparse.ArgumentParser(description='Buy top gaining stocks with GTT OCO protection')
    parser.add_argument('--execute', action='store_true', help='Actually place orders (default is dry run)')
    parser.add_argument('--top', type=int, default=5, help='Number of top gainers to buy (default: 5)')
    parser.add_argument('--amount', type=float, default=25000, help='Amount per stock in Rs (default: 25000)')
    parser.add_argument('--min-gain', type=float, default=0.5, dest='min_gain', help='Minimum gain %% to consider (default: 0.5)')
    ...
    gainers = get_top_gainers(kite, symbols)
    ...
    for i, g in enumerate(gainers[:20], 1):  # Show top 20
    ...
    # Filter by min gain
    qualified = [g for g in gainers if g['change_pct'] >= min_gain_pct]
    
    if not qualified:
        print(f"\nNo stocks with gain >= {min_gain_pct}%")
        return
    ...
    print()
    print("=" * 80)
    print(f"BUYING TOP {len(to_buy)} GAINERS (min {min_gain_pct}% gain)")
    print("=" * 80)
    ...
    print()
    print("-" * 100)
    print(f"{'Symbol':<15} {'Qty':>6} {'Price':>12} {'Amount':>15} {'Gain%':>10} {'SL':>12} {'Target':>12}")
    print("-" * 100)
