...
try:
    from src.cloud_api_client import CloudAPIClient, CloudAPIConfig, get_cloud_client
...
def display_ai_chat(client: CloudAPIClient):
    ...
    with col1:
        model = st.selectbox(
            "Model",
            ["gemini-2.0-flash", "gemini-2.5-flash-lite", "gemini-1.5-pro"],
            key="chat_model"
        )
...
def display_tips_research_v3():
    ...
    with col2:
        ...
        st.subheader("🤖 Cloud AI Enhancement")
        
        if client and tips_path.exists():
            if st.button("🧠 Run Cloud AI Analysis", use_container_width=True, key="cloud_enhance"):
                with st.spinner("Running Cloud AI analysis on tips stocks..."):
                    try:
                        tips_df = pd.read_csv(tips_path)
                        ...
                        if result.get("status") == "success":
                            st.success("Cloud AI analysis complete!")
                            
                            # Save analysis
                            report_path = REPO_ROOT / "data" / "cloud_ai_tips_analysis.md"
                            ai_response = result.get("result", {}).get("response", "")
                            
                            with open(report_path, 'w', encoding='utf-8') as f:
                                f.write(f"# Cloud AI Tips Analysis\n\n")
                                f.write(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                                f.write("---\n\n")
                                f.write(ai_response)
                            
                            st.info(f"📄 Saved to {report_path.name}")
                        else:
                            st.error(f"Failed: {result.get('error')}")
                    except Exception as e:
                        st.error(f"Error: {e}")
        else:
            st.info("Generate tips CSV first, then run Cloud AI analysis")
