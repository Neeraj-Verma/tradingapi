...
class CloudAPIClient:
    ...
    def chat(self, message: str, system_prompt: Optional[str] = None, 
             model: str = "gemini-2.5-flash", use_vertex: bool = False) -> Dict[str, Any]:
        """
...
- Overall portfolio strategy"""
    ...
    def tips_research(self, stocks: List[Dict[str, Any]], max_stocks: int = 10, 
                      search_per_stock: int = 5) -> Dict[str, Any]:
        """
        Research stocks using web search and AI analysis (uses research sources)
        
        Args:
            stocks: List of stock dicts with Symbol, Price, Rank, etc.
            max_stocks: Maximum stocks to analyze
            search_per_stock: Number of search results per stock
            
        Returns:
            Research data and AI recommendations with rationale
        """
        return self._request("POST", "/tips-research", json={
            "stocks": stocks,
            "max_stocks": max_stocks,
            "search_per_stock": search_per_stock
        })
